    ...
    <div className="h-full flex flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {viewMode === 'markdown' ? (
          ...
          <ProcessTemplateView template={template as ProcessTemplate} />
        ) : (
...
function ProcessTemplateView({ template }: { template: ProcessTemplate }) {
  return (
      ...
      <Section title={`Steps (${template.steps.length})`}>
        <div className="space-y-2">
          {template.steps.map((step, index) => (
            <div key={index} className="flex items-start gap-3 p-2 bg-surface rounded-md">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-text-muted mt-0.5">
                  {getStepRefDisplayName(step.stepRef)}
                </p>
                <p className="text-[10px] text-text-muted mt-0.5">
...
function StepTemplateView({ template }: { template: StepTemplate }) {
  return (
    <div className="space-y-6">
      {/* Metadata */}
      <Section title="Overview">
